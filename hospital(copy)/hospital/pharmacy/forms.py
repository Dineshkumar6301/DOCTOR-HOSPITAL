...
from .models import Appointment, Patient, Doctor, Clinic, Education, Experience,TimeSlot,Service,Award,Speciality,Branch,ClinicListing
...
import re
from .models import BloodBank, BloodInventory, BloodRequest, DonorRecord, BloodDispatch 
...
class DoctorProfileForm(forms.ModelForm):
    ...
    clinics = forms.ModelMultipleChoiceField(
        ...
        required=False,
        help_text='Hold Ctrl or Cmd to select multiple clinics.'
    )
    class Meta:
        model = Doctor
        fields = [
            ...
            'zip_code', 'country', 'qualification', 'facebook_url', 
            'twitter_url', 'google_plus_url', 'instagram_url', 'clinics'
        ]
        widgets = {
            ...
            'google_plus_url': forms.URLInput(attrs={'placeholder': 'https://plus.google.com'}),
...
class ClinicListingForm(forms.ModelForm):
    class Meta:
        model = ClinicListing
        fields = ['treatment', 'price']
        widgets = {
...
class BloodBankForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'affiliated_hospitals': forms.Textarea(attrs={'rows': 2}),
        }

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.instance and self.instance.services:
            self.initial['services'] = self.instance.services

    def clean_services(self):
        return self.cleaned_data.get('services', [])
...
class BloodInventoryForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'testing_status': forms.Select(attrs={'class': 'form-select'}),
...
class BloodRequestForm(forms.ModelForm):
    ...
    class Meta:
        model = BloodRequest
        exclude = ['created_at', 'updated_at', 'status','requested_by']

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['blood_bank'].required = False
...
class DonorRecordForm(forms.ModelForm):
    ...
    blood_bank_name = forms.ModelChoiceField(
        queryset=BloodBank.objects.all(),
        label="Blood Bank / Hospital Name",
        help_text="Select the blood bank you are donating to.",
        empty_label="-- Select a Blood Bank --",
        widget=forms.Select(attrs={'class': 'form-select'})
    )

    class Meta:
        model = DonorRecord
        exclude = ['blood_bank', 'created_at', 'updated_at', 'requested_by']


class DispatchBloodForm(forms.ModelForm):
    class Meta:
        model = BloodDispatch
        fields = ['blood_group', 'quantity', 'recipient_name', 'purpose']
...
class DiagnosticTestForm(forms.ModelForm):
    class Meta:
        ...
        fields = '__all__' 
        widgets = {
            'sample_type': forms.SelectMultiple(attrs={'class': 'form-select'}),
        }
...
class TestCategoryForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'type': forms.Select(attrs ={'placeholder':'Select' })
        }
...
class ProductForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['in_stock'].label = "In Stock"